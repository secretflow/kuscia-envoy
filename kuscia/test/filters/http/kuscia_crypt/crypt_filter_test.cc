...
class CryptFilterTest : public testing::Test {
  public:
    ...
    void checkEncryptionParams(bool forward_encrypt, absl::string_view forward_key, bool reverse_encrypt,
                               absl::string_view reverse_key) {
...
TEST_F(CryptFilterTest, RequestAndResponse) {
    ...
    EXPECT_EQ(data.length(), request_body.length());
    ...
    EXPECT_EQ(Http::FilterHeadersStatus::Continue, peer_filter_.decodeHeaders(request_headers, false));
    ...
    EXPECT_EQ(Http::FilterHeadersStatus::Continue, peer_filter_.encodeHeaders(response_headers, false));
    ...
    EXPECT_EQ(data.length(), response_body.length());
...
TEST_F(CryptFilterTest, BigRequestBody) {
    ...
    EXPECT_EQ(kBodySize, encrypted_body.length());
...
TEST_F(CryptFilterTest, NoEncrypt) {
    ...
    EXPECT_EQ(Http::FilterHeadersStatus::Continue, filter_.decodeHeaders(request_headers, false));
    KusciaHeaderChecker::checkRequestHeaders(request_headers,
    ExpectHeaders{{kEncryptVersion, ""}});
