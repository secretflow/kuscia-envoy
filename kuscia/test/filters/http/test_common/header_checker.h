...
public:
  static void checkRequestHeaders(const Http::TestRequestHeaderMapImpl& headers,
    ...
    for (const auto& iter : expects) {
      auto result_sv = headers.get(Http::LowerCaseString(iter.key));
      absl::string_view result;
      if (!result_sv.empty()) {
        result = result_sv[0]->value().getStringView();
      }
      EXPECT_EQ(result == iter.value, iter.equal);
    }
  }

  static void checkResponseHeaders(const Http::TestRequestHeaderMapImpl& headers,
                                   const ExpectHeaders& expects) {
    for (const auto& iter : expects) {
