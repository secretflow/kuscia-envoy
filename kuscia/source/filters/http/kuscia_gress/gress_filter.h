...
public:
  ...
  RewritePolicy rewritePolicy() const { return rewrite_policy_; }
  const std::string& specifiedHost() const { return specified_host_; }

  const std::vector<PathMatcherConstSharedPtr>& pathMatchers() const { return path_matchers_; }
...
public:
  ...
  const std::string& selfNamespace() const { return self_namespace_; }

  bool addOriginSource() const { return add_origin_source_; }

  int32_t maxLoggingBodySizePerReqeuest() { return max_logging_body_size_per_reqeuest_; }

  const std::vector<RewriteHostConfig>& rewriteHostConfig() const { return rewrite_host_config_; }
...
public:
  explicit GressFilter(GressFilterConfigSharedPtr config)
      : config_(config), host_(), request_id_(), record_request_body_(false),
...
private:
  ...
  std::string getRichMessage(const StreamInfo::StreamInfo& stream_info,
