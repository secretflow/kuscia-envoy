...
Http::FilterHeadersStatus GressFilter::decodeHeaders(Http::RequestHeaderMap& headers, bool) {
  ...
  auto record = headers.get(KusciaCommon::HeaderKeyRecordBody);
  if (!record.empty() && std::string(record[0]->value().getStringView()) == "true") {
...
bool GressFilter::rewriteHost(Http::RequestHeaderMap& headers, const RewriteHostConfig& rh) {
  auto value = headers.get(Http::LowerCaseString(rh.header()));
  if (value.empty()) {
    return false;
  }
  absl::string_view header_value = value[0]->value().getStringView();
