...
static const int AES_GCM_TAG_LENGTH = 16; // GCM recommend tag len
static const int AES_GCM_IV_LENGTH = 12;  // GCM recommend iv len
...
public:
  ...
  KusciaCrypter(const std::string& secret_key, const std::string& version)
      : secret_key_(secret_key), version_(version) {}

  KusciaCrypter(std::string&& secret_key, std::string&& version)
      : secret_key_(std::move(secret_key)), version_(std::move(version)) {}
